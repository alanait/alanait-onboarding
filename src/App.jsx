...
// Escapa valores introducidos por el técnico para que no rompan el HTML del informe
const esc = (s) => String(s ?? "").replace(/[&<>"']/g, c => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[c]));

// CSS del informe. Va acotado a .alana-print para no afectar a la app cuando se
// inyecta en el contenedor temporal que usa html2pdf.
const PRINT_CSS = `
  .alana-print, .alana-print * { box-sizing: border-box; }
  .alana-print { font-family: 'Segoe UI', Arial, sans-serif; color: #1e293b; }
  /* table-layout fijo + corte de palabra: evita que un texto largo sin espacios
     (URLs, rutas, notas) ensanche la tabla y salga cortada por la derecha */
  .alana-print table { border-collapse: collapse; width: 100%; table-layout: fixed; }
  .alana-print td, .alana-print th { word-wrap: break-word; overflow-wrap: anywhere; }
  .alana-print tr, .alana-print td { page-break-inside: avoid !important; break-inside: avoid !important; }
  .alana-print h2 { page-break-after: avoid !important; break-after: avoid !important; }
  .alana-print img { max-width: 100% !important; height: auto !important; page-break-inside: avoid !important; break-inside: avoid !important; }
...
function buildPrintHTML(clientData, sectionEnabled, formData, instanceCounts, sectionImages) {
  ...
  body += `<div style="page-break-after:always;padding:60px 48px 48px;box-sizing:border-box;background:#fff;">
    ...
    <h1 style="font-size:52px;font-weight:800;color:#0d1f3c;margin:0 0 12px 0;line-height:1.1;">${esc(clientData.empresa || "Empresa")}</h1>
    ...
    <table style="border-collapse:collapse;width:auto;margin-bottom:60px;">
      ${clientData.contacto ? `<tr><td style="padding:6px 20px 6px 0;font-size:13px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;">Contacto</td><td style="padding:6px 0;font-size:14px;color:#1e293b;font-weight:500;">${esc(clientData.contacto)}</td></tr>` : ""}
      ${clientData.telefono ? `<tr><td style="padding:6px 20px 6px 0;font-size:13px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;">Teléfono</td><td style="padding:6px 0;font-size:14px;color:#1e293b;">${esc(clientData.telefono)}</td></tr>` : ""}
      ${clientData.email ? `<tr><td style="padding:6px 20px 6px 0;font-size:13px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;">Email</td><td style="padding:6px 0;font-size:14px;color:#1e293b;">${esc(clientData.email)}</td></tr>` : ""}
      ${clientData.web ? `<tr><td style="padding:6px 20px 6px 0;font-size:13px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;">Web</td><td style="padding:6px 0;font-size:14px;color:#1d4ed8;">${esc(clientData.web)}</td></tr>` : ""}
      ${clientData.direccion ? `<tr><td style="padding:6px 20px 6px 0;font-size:13px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;">Dirección</td><td style="padding:6px 0;font-size:14px;color:#1e293b;">${esc(clientData.direccion)}</td></tr>` : ""}
      ${clientData.fecha ? `<tr><td style="padding:6px 20px 6px 0;font-size:13px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;">Fecha visita</td><td style="padding:6px 0;font-size:14px;color:#1e293b;">${esc(clientData.fecha)}</td></tr>` : ""}
      ${clientData.responsable ? `<tr><td style="padding:6px 20px 6px 0;font-size:13px;color:#94a3b8;font-weight:600;text-transform:uppercase;letter-spacing:0.05em;">Técnico</td><td style="padding:6px 0;font-size:14px;font-weight:600;color:#1d4ed8;">${esc(clientData.responsable)}</td></tr>` : ""}
    </table>
  ...
  body += `<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;padding-bottom:14px;border-bottom:3px solid #0d1f3c;">
    <div>
      ...
      <h1 style="margin:0;font-size:24px;color:#0d1f3c;font-weight:800;">${esc(clientData.empresa || "—")}</h1>
    </div>
    ...
    if (imgs.length > 0) {
      ...
      imgs.forEach(img => {
        imgHtml += `<div style="margin-bottom:12px;border:1px solid #e2e8f0;border-radius:6px;overflow:hidden;page-break-inside:avoid;">
          ...
          ${img.caption ? `<p style="font-size:11px;color:#64748b;padding:5px 8px;margin:0;background:#f8fafc;">${esc(img.caption)}</p>` : ""}
        </div>`;
      });
      imgHtml += `</div>`;
    }
  ...
  if (otherNotes || otras.length > 0) {
    ...
    if (otherNotes) {
      body += `<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:6px;padding:12px;margin-bottom:12px;font-size:12px;white-space:pre-wrap;">${esc(otherNotes)}</div>`;
    }
    otras.forEach(img => {
  ...
  // El <style> va DENTRO del <body>: handlePrint extrae solo el contenido del body
  // para pasarselo a html2pdf, asi que un <style> en el <head> se perdia y con el
  // todas las reglas de ancho y de salto de pagina.
  return `<!DOCTYPE html><html lang="es"><head><meta charset="UTF-8">
    ...
    <div class="alana-print">${body}</div>
  </body></html>`;
}
